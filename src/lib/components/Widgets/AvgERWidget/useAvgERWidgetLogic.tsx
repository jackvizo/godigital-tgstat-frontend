...
export const AVG_ER_QUERY = graphql(`
  query ERAvg($from_date: timestamp!, $to_date: timestamp!, $tg_channel_ids: [bigint!]) {
    stat_channel_aggregate(
      where: { timestamp: { _gte: $from_date, _lte: $to_date }, tg_channel_id: { _in: $tg_channel_ids } }
    ) {
      aggregate {
        avg {
          total_participants
...
export function useAvgERWidgetLogic(props: UseAvgERWidgetLogicProps) {
  ...
  const usersCount = Number(avgErQuery?.data?.stat_channel_aggregate?.aggregate?.avg?.total_participants);
